...
router.put("/api/profile/follow/:id", auth, async (req, res) => {
  try {
    ...
    const isAlreadyFollowed = userProfile.followers.find(
...
router.delete("/api/profile/unfollow/:id", auth, async (req, res) => {
  try {
    ...
    const isFollowed = userProfile.followers.find(
      (follower) => follower.user.toString() === req.user.id
    );
    if (!isFollowed) {
      return res
        .status(400)
        .send({ err: "You are already not following this profile" });
    }
    ...
    const indexOfFollower = userProfile.followers
    ...
    const indexOfFollowee = myProfile.following
...
//API ROUTE TO GET A SPECIFIC PROFILE BY USERNAME
router.get("/api/profile/:username", async (req, res) => {
  try {
    const user = await User.findOne({ username: req.params.username });
    if (!user) {
      res.status(404).send({ err: "The user does not exists" });
    }
